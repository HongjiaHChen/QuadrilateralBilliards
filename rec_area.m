function y = rec_area(L_H, L_V, alpha0, position0, period)
% Computes the 'area' of a billiard trajectory within the rectangle.
% User specifies the initial angle 'alpha0' and initial position 'P0' as well
% as the period of the periodic orbit that these initial conditions yield/

alpha = zeros(1, period+1); side = zeros(1, period+1); position = zeros(1, period+1);
...
for i=1:period
    ...
    if x_i < eps  % hit a vertex
        ...
        period = i; % for plotting purposes
        break
    end
    ...
    %if abs(side(i)-0) < eps || abs(side(i) - (L_H + L_V)) < eps  % Bottom and top sides of length L_H
        ...
        if simplify(vpa(0) < alpha(i) & alpha(i) < atan(L_V/(L_H-x_i)))  % right adjacent side
            ...
            area = area + 0.5 * (L_H - (position(i)-side(i))) * (position(i+1) - side(i+1));
        ...
        elseif simplify(atan(L_V/(L_H-x_i)) < alpha(i)) & simplify(alpha(i) < pi-atan(L_V/x_i)) % opposite side
            ...
            %area = area + 0.5 * (L_H - (position(i)-side(i)) + (position(i+1) - side(i+1)))*L_V;
        ...
        elseif simplify(pi-atan(L_V/x_i) < alpha(i) & alpha(i) < vpa(pi))  % left adjacent side
            ...
            %area = area + L_H*L_V - 0.5 * (position(i)-side(i)) * (L_V - (position(i+1) - side(i+1)));
            
            area = area + 0.5 * (position(i)-side(i)) * (L_V - (position(i+1) - side(i+1))); % NEW
    ...
    %elseif abs(side(i)-L_H)<eps || abs(side(i)-(2*L_H+L_V))<eps  % Left and right sides with length L_V
        ...
        if simplify(vpa(0) < alpha(i) & alpha(i) < atan(L_H/(L_V-x_i)))  % right adjacent side
            ...
            area = area + 0.5 * (L_V - (position(i)-side(i))) * (position(i+1) - side(i+1));
        ...
        elseif simplify(pi-atan(L_H/x_i) < alpha(i) & alpha(i) < vpa(pi))  % left adjacent side
            ...
            %area = area + L_H*L_V - 0.5 * (position(i)-side(i)) * (L_H - (position(i+1) - side(i+1)));
            
            area = area + 0.5 * (position(i)-side(i)) * (L_H - (position(i+1) - side(i+1))); % new
    ...
    %area = area + 0.5 * (L_H - (position(i)-side(i))) * (position(i+1) - side(i+1));   % TEMPORARY
...
y = period*L_V*L_H-area;   % this makes sure the output y is positive
end
